class RelationshipsController < ApplicationController
  before_action :current_user_must_be_relationship_sitter, :only => [:show, :edit_form, :update_row, :destroy_row]

  def current_user_must_be_relationship_sitter
    relationship = Relationship.find(params["id_to_display"] || params["prefill_with_id"] || params["id_to_modify"] || params["id_to_remove"])

    unless current_user == relationship.sitter
      redirect_to :back, :alert => "You are not authorized for that."
    end
  end

  before_action :current_user_must_be_relationship_home_owner, :only => [:show, :edit_form, :update_row, :destroy_row]
